...
from fbprophet import Prophet  # need to installed 'pystan==2.17.1.0'
from termcolor import colored
# import pypng # for QR code
import os
...
file = 'whereikeepinfo\\views\\data\\mx_us.csv'
...
@view_defaults(route_name='entry')
...
def datasetServer(request):
    ...
    if (format == 'csv'):
        ...
        r = Response(base64.encodebytes(pkl.dumps(getDataframe())).decode('utf-8'), content_type='text/html',
                     charset='UTF-8')
    elif (format == 'excel'):
        b = BytesIO()
        pd.ExcelWriter(b)
...
def chartServer(request):
    ...
    if dpi == -1 or chartType == 'not' or format == 'not' or days == -1:
        ...
        if chartType == 'data':
            ...
            plt.clf()
            img = plt.imread("whereikeepinfo\\views\\matplotlib.png")
            plt.plot(df.date, df.mx_usd, 'r')
            fig = plt.gcf()
            fig.set_size_inches(18.5, 10.5)
            plt.title('Forecast chart');
            ax = plt.axes([.60, -0.13, 0.25, 0.8], frameon=True)

            ax.imshow(img, alpha=0.112)
            ax.axis('off')
            figure = BytesIO()
            print('Output data: Forecast chart on site. ')
            plt.savefig(figure)  # was format=format,dpi=dpi
    ...
    if format == 'png':
        ...
        content_type = "image/jpg"  # was "image/jpg"
...
def qrServer(request):
    ...
    if scale == -1:
        ...
        oRet = BytesIO()
        oQR = pyqrcode.create('http://' + socket.gethostbyname(socket.gethostname()) + ':6543/hello')
...
if __name__ == '__main__':
    ...
    # W.printWelcome()
    print('*' * 40)
    with Configurator(root_factory=BearFactory) as config:
        # For localhost:port/admin/
        ...
        # Добовляем стороние модули к проэкту
        config.include('pyramid_jinja2')  # поддержка jinja2 шаблонов

        # добовляем маршруты(роуты-гиперссылки) на страницу
        ...
        # Make server and WSGI application
